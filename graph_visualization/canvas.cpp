...
void Canvas::paintEvent(QPaintEvent * pe)
{
    if(data) {
        ...
        QVector<QPoint> vertx_points = getGraphVertices(data);
        ...
        for(int i =0; i!=data->spanning_tree.size();++i) {
            ...
            circle_rect.setRect(vertx_points[i].x()-circle_radius/2, vertx_points[i].y()-circle_radius/2,circle_radius,circle_radius);
            painter.drawEllipse(circle_rect);
            painter.drawText(vertx_points[i] + QPoint(-10,-20),QString::fromStdString(data->valids[i]));
            painter.setPen(Qt::red);
            painter.drawText(circle_rect, Qt::AlignCenter,QString::number(i+1));
            painter.setPen(Qt::black);
        }
    }
}

QVector<QPoint> Canvas::getGraphVertices(const graph_data* dg)
{
    //debug
    for(auto &x: dg->spanning_tree) {
        qDebug()<<x;
    }
    //\debug
    ...
    int horizontal_inc = 0, vertical_inc = this->height()/(dg->morse_line.size() +1);
    while(!current_lvl.isEmpty()){
        int node = current_lvl.dequeue();
        for(int i = node; i!=dg->valids.size();++i) {
