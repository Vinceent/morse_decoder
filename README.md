...
sh installation.sh
```
После завершения installation.sh в дирректории появятся 5 новых файлов:
...
* **test_code** - Тестовый файл ввода с готовым случайным набором последовательностей.
* **gen_test_code.py** - Скрипт для генерации файла **test_code**. Запуск осуществляется командой *python gen_test_code.py*. В результате своей работы создает файл
 **test_code**, содержащий набор случайных последовательностей морзе.
